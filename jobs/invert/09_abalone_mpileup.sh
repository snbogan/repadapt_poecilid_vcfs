...
#SBATCH --array=1-616
#SBATCH --output=09_ab_mpileup_out/09_ab_mpileup_out_%A_%a.out
#SBATCH --error=09_ab_mpileup_err/09_ab_mpileup_err_%A_%a.err
...
CHROM=$(sed -n "${SLURM_ARRAY_TASK_ID}p" chromosomes.txt) ### list of chromosomes (can be found in the FASTA index file of reference genome -- .fai file). This species has 616, that's why array number is 616. We parallelize by chromosome.
...
bcftools mpileup -Ou -f /home/snbogan/repadapt/vcf/invert/genomes/abalone/GCA_023055435.1_xgHalRufe1.0.p_genomic.fna --bam-list /home/snbogan/repadapt/vcf/invert/08_cnv/abalone/08_list1.txt -q 5 -r $CHROM -I -a FMT/AD,FMT/DP | bcftools call -S ploidymap.txt -G - -f GQ -mv -Ov > $CHROM\.vcf
