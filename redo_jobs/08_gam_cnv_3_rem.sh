...
#SBATCH --job-name=08_gam_depth_table
#SBATCH --output=08_gam_depth_table.out
#SBATCH --error=08_gam_depth_table.err
...
find . -name "*realigned.bam" | \
    ...
    sed 's/^..//g' | \
    grep -v -E '^(EFR22_278_S3_L004|EFR22_292_S14_L004)$' > list.txt

echo -e "location\t$(cut -f2 list.txt | sort | uniq | paste -s -d '\t')" > depthheader.txt

# get a list of sample names
cut -f2 list.txt | sort | uniq > /data/colibri/kelley_lab/bogan/RepAdapt/08_cnv/gam/samples.txt
