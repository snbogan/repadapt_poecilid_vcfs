...
#SBATCH --qos=pi-jkoc
#SBATCH --job-name=09_gam_mpileup
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --array=1-77
#SBATCH --output=09_gam_mpileup_out/09_gam_mpileup_out_%A_%a.out
#SBATCH --error=09_gam_mpileup_err/09_gam_mpileup_err_%A_%a.err

module load miniconda3
conda activate bcftools_v1.16

cd /data/colibri/kelley_lab/bogan/RepAdapt/09_mpileup/gam/

CHROM=$(sed -n "${SLURM_ARRAY_TASK_ID}p" chromosomes.txt) ### list of chromosomes (can be found in the FASTA index file of reference genome -- .fai file). This species has 77, that's why array number is 77. We parallelize by chromosome.
...
bcftools mpileup -Ou -f /data/colibri/kelley_lab/bogan/RepAdapt/02_bwa_index/gam/rawg0043_Gambusia_sexradiata_kelley.fasta --bam-list /data/colibri/kelley_lab/bogan/RepAdapt/08_cnv/gam/08_list1_redo.txt -q 5 -r $CHROM -I -a FMT/AD,FMT/DP | bcftools call -S ploidymap.txt -G - -f GQ -mv -Ov > $CHROM\.vcf
