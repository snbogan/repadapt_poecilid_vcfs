...
#SBATCH --qos=pi-jkoc
#SBATCH --job-name=09_poec_mpileup
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --array=1-76
#SBATCH --output=09_poec_mpileup_out/09_poec_mpileup_out_%A_%a.out
#SBATCH --error=09_poec_mpileup_err/09_poec_mpileup_err_%A_%a.err
...
CHROM=$(sed -n "${SLURM_ARRAY_TASK_ID}p" chromosomes.txt) ### list of chromosomes (can be found in the FASTA index file of reference genome -- .fai file). This species has 76, that's why array number is 76. We parallelize by chromosome.
...
bcftools mpileup -Ou -f /data/colibri/kelley_lab/bogan/RepAdapt/02_bwa_index/poec/rawg0041_Poecilia_mexicana_kelley.fasta --bam-list /data/colibri/kelley_lab/bogan/RepAdapt/08_cnv/poec/08_list1_redo.txt -q 5 -r $CHROM -I -a FMT/AD,FMT/DP | bcftools call -S ploidymap.txt -G - -f GQ -mv -Ov > $CHROM\.vcf
